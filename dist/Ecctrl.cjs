...
const JoystickComponents = (props) => {
  ...
  if (!joystickDiv) throw new Error("Joystick div not found");
  ...
  const joystickBaseGeo = React.useMemo(
    () => new THREE__namespace.CylinderGeometry(2.3, 2.1, 0.3, 16),
    []
  );
  const joystickStickGeo = React.useMemo(
    () => new THREE__namespace.CylinderGeometry(0.3, 0.3, 3, 6),
    []
  );
  const joystickHandleGeo = React.useMemo(
    () => new THREE__namespace.SphereGeometry(1.4, 8, 8),
    []
  );
  ...
  React.useEffect(() => {
    ...
    if (!viewport) throw new Error("Visual viewport not found");
    viewport.addEventListener("resize", onWindowResize);
    return () => {
      joystickDiv.removeEventListener(
        "touchmove",
        onTouchMove
      );
  ...
  return /* @__PURE__ */ React.createElement(React.Suspense, { fallback: "null" }, /* @__PURE__ */ React.createElement(
...
const ButtonComponents = ({
  ...
  const buttonLargeBaseGeo = React.useMemo(
    () => new THREE__namespace.CylinderGeometry(1.1, 1, 0.3, 16),
    []
  );
  const buttonSmallBaseGeo = React.useMemo(
    () => new THREE__namespace.CylinderGeometry(0.9, 0.8, 0.3, 16),
    []
  );
  const buttonTop1Geo = React.useMemo(
    () => new THREE__namespace.CylinderGeometry(0.9, 0.9, 0.5, 16),
    []
  );
  const buttonTop2Geo = React.useMemo(
    () => new THREE__namespace.CylinderGeometry(0.9, 0.9, 0.5, 16),
    []
  );
  const buttonTop3Geo = React.useMemo(
    () => new THREE__namespace.CylinderGeometry(0.7, 0.7, 0.5, 16),
    []
  );
  const buttonTop4Geo = React.useMemo(
    () => new THREE__namespace.CylinderGeometry(0.7, 0.7, 0.5, 16),
    []
  );
  const buttonTop5Geo = React.useMemo(
    () => new THREE__namespace.CylinderGeometry(0.7, 0.7, 0.5, 16),
    []
  );
  ...
  if (!buttonDiv) throw new Error("Button div not found");
  const [springs, api] = three.useSpring(() => ({
...
const EcctrlJoystick = React.forwardRef(
  (props, ref) => {
